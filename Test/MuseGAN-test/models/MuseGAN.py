...
class MuseGAN():
    def __init__(self, input_dim, critic_learning_rate, generator_learning_rate, optimiser, grad_weight, z_dim, batch_size, n_tracks, n_bars, n_steps_per_bar, n_pitches
                 ):
    ...
    def conv(self, x, f, k, s, a, p):
        x = Conv3D(
            filters=f, kernel_size=k, padding=p, strides=s, kernel_initializer=self.weight_init
        )(x)
        if a == 'relu':
            x = Activation(a)(x)
        elif a == 'lrelu':
    ...
    def BarGenerator(self):
        ...
        input_layer = Input(shape=(self.z_dim * 4,),
                            name='bar_generator_input')
    ...
    def _build_generator(self):
        ...
        chords_over_time = self.chords_tempNetwork(
        ...
        for track in range(self.n_tracks):
            ...
            melody_over_time[track] = self.melody_tempNetwork[track](
                melody_track)
        ...
        for bar in range(self.n_bars):
            ...
            c = Lambda(lambda x: x[:, bar, :], name='chords_input_bar_' +
                       str(bar))(chords_over_time)  # [z_dim]
            s = style_input  # [z_dim]

            for track in range(self.n_tracks):

                m = Lambda(lambda x: x[:, bar, :])(
    ...
    def train_critic(self, x_train, batch_size, using_generator):
        ...
        d_loss = self.critic_model.train_on_batch(
            [true_imgs, chords_noise, style_noise, melody_noise, groove_noise], [valid, fake, dummy])
        return d_loss
    ...
    def plot_model(self, run_folder):
        ...
        plot_model(self.critic, to_file=os.path.join(
    ...
    def save(self, folder):
        ...
        with open(os.path.join(folder, 'params.pkl'), 'wb') as f:
            pickle.dump([
                self.input_dim, self.critic_learning_rate, self.generator_learning_rate, self.optimiser, self.grad_weight, self.z_dim, self.batch_size, self.n_tracks, self.n_bars, self.n_steps_per_bar, self.n_pitches
            ], f)
