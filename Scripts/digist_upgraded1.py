    ...
    def process(self):
        ...
        for chunkIdx in range(len(self.input[0])):
            if (type(self.input[0][chunkIdx]) == OVSignalHeader):
                ...
                if(ratio >= 84):
                    chord_pro = 0
                elif(ratio < 84 and ratio >= 72):
                    chord_pro = 1
                elif(ratio < 72 and ratio >= 60):
                    chord_pro = 2
                elif(ratio < 60 and ratio >= 48):
                    chord_pro = 3
                elif(ratio < 48 and ratio >= 36):
                    chord_pro = 4
                elif(ratio < 36 and ratio >= 24):
                    chord_pro = 5
                elif(ratio < 24 and ratio >= 12):
                    chord_pro = 6
                else:
                    chord_pro = 7
